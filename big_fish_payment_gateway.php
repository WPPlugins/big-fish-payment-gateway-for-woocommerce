	...
	class BigFishPaymentGateway extends WC_Payment_Gateway {
			...
			$this->form_fields = array(
				...
				'providers' => array(
					...
					'options' => array(
						...
						'PayU2' => __('PayU (Poland)', BF_PMGW_PLUGIN),
						'PSC' => __('paysafecard', BF_PMGW_PLUGIN),
						'OTPSimple' => __('OTP Simple', BF_PMGW_PLUGIN),
						'OTPSimpleWire' => __('OTP Simple Wire', BF_PMGW_PLUGIN),
						'Saferpay' => __('Saferpay', BF_PMGW_PLUGIN),
						'IPG' => __('Intelligent Payments', BF_PMGW_PLUGIN),
						'Barion2' => __('Barion Smart Gateway', BF_PMGW_PLUGIN),
					),
				),				
			);
		}
		...
		public function result() {
			...
			try {
				...
				if ($response->ResultCode == "SUCCESSFUL") {
					...
					if ($transaction->provider_name == 'PayPal') {
						$details = \BigFish\PaymentGateway::details(new BigFish\PaymentGateway\Request\Details($_GET["TransactionId"]));
					
						if (isset($details->ProviderSpecificData->OneClickPayment) && !empty($details->ProviderSpecificData->OneClickPayment)) {
							$responseMessage.= "<br/><b>" . __('Recurring payment period', BF_PMGW_PLUGIN) . ":</b> " . __($details->ProviderSpecificData->Extra->REFERENCE->BILLINGPERIOD, BF_PMGW_PLUGIN);
						}
					}
					...
					throw new Exception($response->ResultMessage."<br/>".__('Transaction ID', BF_PMGW_PLUGIN).": ".$response->ProviderTransactionId);
				}
			} catch (Exception $e) {
		...
		public function close_transaction($order_id, $approved) {
			...
			if (!empty($transaction->transaction_id) && !(int)$transaction->auto_commit) {
				...
				$getResult = \BigFish\PaymentGateway::result(new BigFish\PaymentGateway\Request\Result($transaction->transaction_id));
		...
		public function process_payment($order_id) {
			...
			try {
				...
				$request->setProviderName($this->providerName)
						...
						->setModuleName('WP (' . $wp_version . ') WC (' . $woocommerce->version . ')')
				...
				if ($this->providerName == 'MKBSZEP') {
					...
					if (isset($this->settings['gatewayPaymentPage']) && (int)$this->settings['gatewayPaymentPage']) {
						$request->setGatewayPaymentPage(true);
					}

					$request->setMkbSzepCardNumber($_POST['MkbSzepCardNumber'])
				...
				if ((int)$wpdb->insert($wpdb->prefix."woocommerce_".strtolower(BF_PMGW_ID)."_transactions", array('order_id' => $order->id, 'provider_name' => (isset($this->className) ? $this->className : $this->providerName), 'auto_commit' => (int)$this->pmgwData['autoCommit']))) {
